...
function url($k=-1){static$u;$u=$u?:explode('/',trim(preg_replace('/([^\w\/])/i','',current(explode('?',$_SERVER['REQUEST_URI'],2))),'/'));return$k!=-1?v($u[$k]):$u;}
...
function _log($m){file_put_contents('log/.'.date('Y-m-d'),time().' '.$_SERVER['REMOTE_ADDR']." $m\n",FILE_APPEND);}
